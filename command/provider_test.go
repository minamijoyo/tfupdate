...
func TestProviderCommandRunUpdate(t *testing.T) {
	testCases := []struct {
		desc             string
		providerName     string
		version          string
  ...
  required_providers {
    aws = {
      ...
      version = "~> 3.0"
			...
			mockVersions:   []string{"4.0.0", "4.1.0"}, // Mock not used for specific version
			mockError:      nil,
			ok:             true,
			expectedUpdate: true,
		},
		{
			desc:         "latest version update for hashicorp provider",
			providerName: "aws",
			version:      "latest",
			fileContent: `terraform {
  required_providers {
    aws = {
      ...
      version = "~> 3.0"
			...
			mockVersions:   []string{"4.0.0", "4.1.0", "4.2.0"},
			mockError:      nil,
			ok:             true,
			expectedUpdate: true,
		},
		{
			desc:         "latest version update for namespaced provider",
			providerName: "integrations/github",
			version:      "latest",
			fileContent: `terraform {
  required_providers {
    github = {
      ...
      version = "~> 4.0"
			...
			mockVersions:   []string{"5.0.0", "5.1.0"},
			mockError:      nil,
			ok:             true,
			expectedUpdate: true,
		},
  ...
  required_providers {
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			args := []string{"-v", tc.version, tc.providerName, "test.tf"}
			exitCode := cmd.Run(args)

			if tc.ok {
				AssertCommandSuccess(t, exitCode, "ProviderCommand.Run(%v)", args)
				...
				AssertCommandFailure(t, exitCode, "ProviderCommand.Run(%v)", args)
				...
				if tc.expectedErrorMsg != "" {
					AssertUIError(t, ui, tc.expectedErrorMsg, "ProviderCommand.Run(%v)", args)
				}
			}
		})
	}
